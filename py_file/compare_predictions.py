...
def main():
    parser = argparse.ArgumentParser(description='模型预测对比分析')
    parser.add_argument('--pred_csv', required=True, help='模型预测结果文件')
    parser.add_argument('--montel_csv', type=str, default=None, help='Montel 预测文件（可选，不提供时只计算模型指标）')
    ...
    # 检查是否提供 Montel 文件
    has_montel = args.montel_csv is not None
    
    if has_montel:
        # 读取 Montel 数据
        montel_df = pd.read_csv(args.montel_csv)
        ...
        print("未提供 Montel 文件，只计算模型与真实值之间的指标")
        # 提取数据（不需要与 Montel 对齐）
        Y_PRICE = model_df['y'].to_numpy().ravel()
        MODEL_PREDICTION = model_df.iloc[:, -1].to_numpy().ravel()
        MONTEL_PREDICTION = None
        montel_df = None
    ...
    # Montel vs Y_PRICE（如果有 Montel 数据）
    if has_montel:
    ...
    visualize_predictions(
        ...
        Y_PRICE, 
        MODEL_PREDICTION, 
        MONTEL_PREDICTION if has_montel else None, 
        model_name, 
        horizon, 
        args.out_dir,
        has_montel
...
def visualize_predictions(model_df, montel_df, y_true, y_model, y_montel, model_name, horizon, out_dir, has_montel=True):
    """
    可视化真实值与预测值的对比（随机挑选 20 个窗口）
    
    参数:
        model_df: 模型预测数据框（包含时间序列信息）
        montel_df: Montel 预测数据框（可选）
        y_true: 真实值数组
        y_model: 模型预测值数组
        y_montel: Montel 预测值数组（可选）
        model_name: 模型名称
        horizon: 窗口大小（小时）
        out_dir: 输出目录
        has_montel: 是否有 Montel 数据
    """
    # 确保输出目录存在
    ...
    num_samples = min(20, num_windows)  # 最多取 20 个
    ...
    print(f"对比模式：{'模型 vs Montel vs 真实值' if has_montel else '模型 vs 真实值'}")
    ...
    for idx, i in enumerate(selected_windows):
        ...
        if has_montel and y_montel is not None:
        ...
        if has_montel:
            ...
            plt.title(f'Model Prediction vs Actual - Window {i+1}', fontsize=14)
        ...
        # 设置横坐标只显示几个关键点（约每 7 天显示一个）
        num_ticks = min(7, len(window_time))  # 最多显示 7 个时间点
